DROP FUNCTION asset_kpi_full(character varying,character varying,character varying,integer, integer);

CREATE OR REPLACE FUNCTION asset_kpi_full(value_stream_name character varying, machine_id character varying, end_time_string character varying, duration_in_sec integer,idea_run_rate integer)
 RETURNS RECORD
 LANGUAGE plpgsql
 STABLE
AS $$
